...
local function show_mute(msg, target)
  local text = "#Silentlist for : [" ..msg.to.title.. "]\n"
--.."\nMute Documents: "..(redis:get("mute:document"..target) or ' ')..""
.."\n#Silent Contacts: "..(redis:get("mute:contact"..target) or ' ')..""
.."\n#Silent Photo: "..(redis:get("mute:photo"..target) or " ")..""
--.."\nMute Video: "..(redis:get("mute:video"..target) or ' ')..""
.."\n#Silent Text: "..(redis:get("mute:text"..target) or ' ')..""
.."\n#Silent Gifs: "..(redis:get("mute:gif"..target) or ' ')..""
.."\n#Silent Audio: "..(redis:get("mute:audio"..target) or ' ')..""
.."\n#Silent All: "..(redis:get("mute:all"..target) or " ")
  return text
end
...
local function pre_process(msg)
  ...
  if msg.text then
    if redis:get("mute:text"..msg.to.id) == " " then
      delete_msg(msg.id, ok_cb, false)
    end
  end
  if msg.media then
    if msg.media.type == "photo" then
      if redis:get("mute:photo"..msg.to.id) == " " then
        delete_msg(msg.id, ok_cb, false)
      end
    end
  end
  if msg.media then
    if msg.media.type == "audio" then
          if redis:get("mute:audio"..msg.to.id) == " " and not is_momod(msg) then
            delete_msg(msg.id, ok_cb, true)
          end
        end
  end
  if msg.media then
    if msg.media.type == "video" then
      if redis:get("mute:video"..msg.to.id) == " " then
        delete_msg(msg.id, ok_cb, false)
      end
    end
  end
  if msg.media then
    if msg.media.caption == "giphy.mp4" and not is_momod(msg) then
          if redis:get("mute:gif"..msg.to.id) == " " then
            delete_msg(msg.id, ok_cb, true)
          end
        end
        if msg.media.type == "contact" and not is_momod(msg) then
          if redis:get("mute:contact"..msg.to.id) == " " then
            delete_msg(msg.id, ok_cb, true)
          end
        end
  end
  if msg.media then
    if msg.media.type == "document" and not msg.media.caption and not is_momod(msg) then
          if redis:get("mute:document"..msg.to.id) == " " then
            delete_msg(msg.id, ok_cb, true)
          end
        end
  end
